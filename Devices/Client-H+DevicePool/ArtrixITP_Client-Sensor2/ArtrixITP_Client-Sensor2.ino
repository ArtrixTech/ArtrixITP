...
bool getHumanSensor(bool last) {
  ...
  bool res = !digitalRead(6);
  delay(5);
  if (!digitalRead(6) == res)return res;
  else return last;
...
void loop() {
  ...
  if (client.handShaked) {
    ...
    if (secondSend && millis() - firstSend >= 125) {
    ...
    if (millis() - lastTemp > 500) {
      ...
      if (sendID == 0) sendUPDSENSOR("TEMP", "FLOAT", (float)bmp.readTemperature());

      if (sendID == 1) sendUPDSENSOR("PRES", "FLOAT", (float)(bmp.readPressure() * 0.01));

      //VOLT float volt = analogRead(A2) / 1024.0 * 5.0;
      //VOLT Serial.println(analogRead(A2));
      //VOLT volt = volt > 4.2 ? 4.2 : volt;
      //VOLT sendUPDSENSOR("BATT", "FLOAT", (float) analogRead(A2));
  ...
  if (!client.handShaked)delay(2000);
  else delay(5);
}
