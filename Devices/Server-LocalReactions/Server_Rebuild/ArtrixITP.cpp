...
String ArtrixITP::getSerial433() {
  ...
  if (!Serial433) {
    this->Serial433.enableIntTx(true);
    this->Serial433.begin(9600);
  }
  while (!Serial433);
  ...
  while (this->Serial433.available() > 0) {
    ...
    if (!this->Serial433.available())delay(3);
    //delay(1);
  }
  strRecv.replace("\n", "");
...
void ArtrixITP::processAllMessage()
{
  ...
  if (this->inited) {
    ...
    if (!WTReturn.length() == 0)
    {
      ...
      cmds.prioritySort("UPDSENSOR", 3, "HUMANSENSOR");
      ...
      while (doCount) {
        ...
        */
        ...
        Reaction match = this->matchReactionByCondition(command.deriveMsg);             // ---------Local Reaction Process---------
        if (match.inited) {

          Serial.print("---------Get Match Reaction:");
          Serial.println(match.getReaction());
          this->send433(match.getReaction());

        }                                                                               // ---------Local Reaction Process---------

        else {     //Based on the connection, handle other commands
...
int checkDelay = random(2500, 5500);  //Millisecond
bool isSend = false;
int nowSendIndex = 0;
int nextSendInterval = 250;
...
void ArtrixITP::SERaliveRequest433()                                     //A function to iterate all devices in the pool and access SEND_ALIVE(DEVICE_ID) to send ALIVE command
{
  if (!isSend) {
    if (millis() - this->lastAliveCheck > checkDelay) {
      ...
      isSend = true;
      checkDelay =  random(2500, 4500);

    }
  }
  else {

    if (nowSendIndex < this->devPool.deviceCount) {

      if (millis() - lastSendTimeStamp > nextSendInterval) {
        SEND_ALIVE(this->devPool.allDevice[nowSendIndex].DEVICE_ID);
        nowSendIndex++;
        nextSendInterval = random(150, 350);
        //Serial.print("SENDALIVE!!!!!");
        //Serial.println(nowSendIndex);
        //Serial.print("Millis!!!!!");
...
void ArtrixITP::SERaliveStateUpdate()                                    //To update the online state by the property "lastAlive"  (This Function is for Server)
{
  for (int i = 0; i < this->devPool.deviceCount; i++) {
    ...
    int aliveThreshold = 12000; //Millisecond
